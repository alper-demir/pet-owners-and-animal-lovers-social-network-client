...
const AdoptionNoticeDetail = () => {
    ...
    const fetchAdoptionNotice = async () => {
        setLoading(true)
        try {
            const response = await axios.get(`${BASE_URL}/adoption-notice/${id}`);
            setAdoptionNotice(response.data);
    ...
    const handleOpen = () => {
        setOpen(true);
        setName(adoptionNotice.name)
        setSpecies(adoptionNotice.species)
        setAge(adoptionNotice.age)
        setGender(adoptionNotice.gender)
        setBreed(adoptionNotice.breed)
        setCity(adoptionNotice.city)
        setColor(adoptionNotice.color)
        setDateOfBirth(adoptionNotice.dateOfBirth)
        setAdoptionStatus(adoptionNotice.adoptionStatus)
        setContactPhoneNumber(adoptionNotice.contactPhoneNumber)
        setDescription(adoptionNotice.description)
    }
    ...
    const handleAdoptionNoticeUpdate = async () => {
        ...
        try {
            const updateNotice = await axios.put(`${BASE_URL}/adoption-notice/${id}`, formData, {
                headers: {
    ...
    const handleDeleteAdoptionNotice = async () => {
        const deleteConfirm = window.confirm("Are you sure delete this adoption notice?");
        if (deleteConfirm) {
            try {
                const deleteNotice = await axios.delete(`${BASE_URL}/delete-adoption-notice/${id}`, { headers: { Authorization: token } });
                if (deleteNotice.data.status) {
                    handleClose();
                    navigate(`/${username}/adoption-notices`)
                ...
                loading ? (
                    ...
                    (adoptionNotice && <>
                        ...
                        <div class="relative overflow-x-auto shadow-md sm:rounded-lg mt-10 mb-20">
                            <table class="w-full text-sm text-left rtl:text-right">
                                <tbody>
                                    <tr class="odd:bg-white odd:dark:bg-[#121212] even:bg-gray-50 even:dark:bg-[#141414] border-b dark:border-[#777777]">
                                        ...
                                        <td class="px-6 py-4">
                                            <img src={`${BASE_URL}/public/images/${adoptionNotice.image}`} alt="img" className='w-60 h-60 object-cover max-sm:w-40 max-sm:h-40 rounded-md max-sm:min-w-40 max-sm:min-h-40' />
                                        </td>
                                    </tr>
                                    <tr class="odd:bg-white odd:dark:bg-[#121212] even:bg-gray-50 even:dark:bg-[#141414] border-b dark:border-[#777777]">
                                        <th scope="row" class="px-6 py-4 font-medium dark:border-[#777777] dark:border-opacity-30 whitespace-nowrap dark:text-white">
                                    ...
                                    <tr class="odd:bg-white odd:dark:bg-[#121212] even:bg-gray-50 even:dark:bg-[#141414] border-b dark:border-[#777777]">
                                        ...
                                        <td class="px-6 py-4">
                                            {new Date(adoptionNotice.dateOfBirth).toLocaleDateString()}
                                        </td>
                                    </tr>
                                    <tr class="odd:bg-white odd:dark:bg-[#121212] even:bg-gray-50 even:dark:bg-[#141414] border-b dark:border-[#777777]">
                                        <th scope="row" class="px-6 py-4 font-medium dark:border-[#777777] dark:border-opacity-30 whitespace-nowrap dark:text-white">
                                    ...
                                    <tr class="odd:bg-white odd:dark:bg-[#121212] even:bg-gray-50 even:dark:bg-[#141414] border-b dark:border-[#777777]">
                                        ...
                                        <td class="px-6 py-4">
                                            {adoptionNotice.contactPhoneNumber}
                                        </td>
                                    </tr>
                                    <tr class="odd:bg-white odd:dark:bg-[#121212] even:bg-gray-50 even:dark:bg-[#141414] border-b dark:border-[#777777]">
                                        <th scope="row" class="px-6 py-4 font-medium dark:border-[#777777] dark:border-opacity-30 whitespace-nowrap dark:text-white">
                            ...
                            <div>
                                {
                                    adoptionNotice.userId._id === userId &&
                            ...
                            <Box className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-2/5 max-xl:w-3/5 max-md:w-4/5 max-sm:w-full max-sm:text-xs p-6 outline-indigo-400 bg-white dark:bg-[#101010] dark:text-white rounded-md overflow-y-auto max-h-[85%]">
                                ...
                                <Typography id="modal-modal-description" sx={{ mt: 2 }} >
                                    ...
                                    <div className="my-2">
                                        ...
                                        <input type="date" id="dateOfBirth" value={new Date(dateOfBirth).toLocaleDateString('en-CA')} onChange={handleLastSeenDateChange} className="bg-gray-50 border border-gray-300 dark:border-[#777777] dark:border-opacity-30 text-sm rounded-lg w-full p-2.5 max-sm:p-2 dark:bg-transparent" />
                                    </div>
                                    <div className="my-2">
                                    ...
                                    <div className="my-2">
                                        <label htmlFor="adoptionStatus" className="block mb-2 text-sm font-medium">Adoption Status</label>

                                        <select id="adoptionStatus" name="adoptionStatus" className="bg-gray-50 border border-gray-300 dark:border-[#777777] dark:border-opacity-30 text-sm rounded-lg w-full p-2.5 max-sm:p-2 dark:bg-[#101010]" value={adoptionStatus} onChange={handleLostStatusChange}>
                                    ...
                                    <div className="flex justify-end bg-white dark:bg-transparent my-3">
                                        <button type="button" class="focus:outline-none text-white bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:focus:ring-yellow-900" onClick={handleAdoptionNoticeUpdate}>Edit Adoption Notice</button>
                                    </div>
                                    <div className="flex justify-end bg-white dark:bg-transparent my-3">
                                        <button type="button" class="focus:outline-none text-white bg-red-500 hover:bg-red-600 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:focus:ring-red-900" onClick={handleDeleteAdoptionNotice}>Delete Notice</button>
                                    </div>
                                </Typography>
                            </Box>
...
export default AdoptionNoticeDetail;