  ...
  return (
    <div className="flex flex-wrap gap-1">
      {posts && posts.map((post, index) => (
        <div key={index} className="w-[32.9%] max-sm:w-full">
          <Link to={`/post/${post._id}`} className="relative group">
            <img src={`${URL}/public/images/${post.image}`} alt={post.title} className="cursor-pointer group-hover:scale-95 duration-300 object-cover rounded-md group-hover:shadow-md group-hover:shadow-indigo-200 group-dark:hover:shadow-[#777777] group-hover:opacity-90" />
